...
getSlashUsage(cmd) {
  let desc = "";
    // Add a description of the command category or group
  if (cmd.category) {
  ...
  if (cmd.slashCommand.options?.find((o) => o.type === ApplicationCommandOptionType.Subcommand)) {
    ...
    subCmds.forEach((sub) => {
      ...
      if (sub.aliases && sub.aliases.length > 0) {
        desc += `❯ **Aliases:** ${sub.aliases.map(alias => `\`${alias}\``).join(", ")}\n`;
      }
      if (sub.usage) {
        desc += `❯ **Usage:** \`\`${sub.usage}\`\`\n`;
      }
      desc += "\n";
    });
  } else {
    ...
    if (cmd.usage) {
      desc += `**Usage:** \`\`${cmd.usage}\`\`\n`;
    }
  }

  // Add command permissions information if available
  if (cmd.slashCommand.permissions && cmd.slashCommand.permissions.length > 0) {
    desc += `\n**Required Permissions:** ${cmd.slashCommand.permissions.map(perm => `\`${perm}\``).join(", ")}`;
  }
...
const embed = new EmbedBuilder()
    ...
    .setAuthor({
    name: 'Darth Bot',
    url: 'https://darthbot.org/',
    iconURL: 'https://i.imgur.com/p6tA3dt.png'
})
 ...
 .setFooter({
    ...
    .setThumbnail('https://i.imgur.com/p6tA3dt.png');
